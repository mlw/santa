...
- (void)testEncodeEntitlements {
  printf("\n\ntestEncodeEntitlements ENTER\n");
  Arena arena;
  NSDictionary *entitlements = @{
    @"com.example.security.test.integer": @(1),
    @"com.example.security.test.float": @(2.2),
    @"com.example.security.test.string": @"hi there",
    @"com.example.security.test.array": @[
      @(3),
      @(4.4),
      @"heya",
    ],
    @"com.example.security.test.dict": @{
      @"com.example.security.test.dict.nested": @{
        @"nested_k1": @[@(1)],
        @"nested_k2": @"Not too nested",
      }
    },
    @"com.example.security.test.date": [NSDate date],
    @"com.example.security.test.bytes": [@"asdf" dataUsingEncoding:NSUTF8StringEncoding],
  };

  ::pbv1::SantaMessage *santa_msg = Arena::CreateMessage<::pbv1::SantaMessage>(&arena);

  EncodeEntitlements(santa_msg->mutable_execution(), entitlements);

  std::string json = ConvertMessageToJsonString(*santa_msg);
  printf("\nGot JSON...\n%s\n\n", json.c_str());

  entitlements = @{ @"a": @{ @"b": @{ @"c": @{ @"d": @{ @"nk5": @"" } } } } };
  santa_msg = Arena::CreateMessage<::pbv1::SantaMessage>(&arena);
  EncodeEntitlements(santa_msg->mutable_execution(), entitlements);
  json = ConvertMessageToJsonString(*santa_msg);
  printf("\nGot JSON...\n%s\n\n", json.c_str());

  // NSMutableArray *arr = [[NSMutableArray alloc] init];
  // for (NSUInteger i = 0; i < 100; i++) {
  //   [arr addObject:[NSString stringWithFormat:@"s-%lu", i]];
  // }
