...
- (void)testLog {
  ...
  auto m = Message(mockESApi, &msg);
  printf("\n\n testLog Create enriched message\n");
  auto enrichedMsg = std::make_shared<EnrichedMessage>(
    EnrichedClose(std::move(m),
    // EnrichedClose(Message(mockESApi, &msg),
                  ...
                  EnrichedFile(std::nullopt, std::nullopt, std::nullopt)));
  printf("\n\n testLog Setup mockSerializer expectations\n");
  EXPECT_CALL(*mockSerializer, SerializeMessage(testing::A<const EnrichedClose &>()))
    .Times(1);
  printf("\n\n testLog Setup mockWriter expectations\n");
  EXPECT_CALL(*mockWriter, Write).Times(1);

  printf("\n\n testLog Create enriched Logger\n");
  auto l = Logger(mockSerializer, mockWriter);
  printf("\n\n testLog Call Log\n");
  l.Log(enrichedMsg);
  // Logger(self->_mockSerializer, self->_mockWriter).Log(enrichedMsg);

  ::testing::Mock::VerifyAndClearExpectations(mockESApi.get());
  ::testing::Mock::VerifyAndClearExpectations(mockSerializer.get());
  ::testing::Mock::VerifyAndClearExpectations(mockWriter.get());
}
...
//   printf("\n\n testLog Create enriched Logger\n");
//   auto l = Logger(self->_mockSerializer, self->_mockWriter);
//   printf("\n\n testLog Call Log\n");
//   l.Log(enrichedMsg);
//   // Logger(self->_mockSerializer, self->_mockWriter).Log(enrichedMsg);

//   ::testing::Mock::VerifyAndClearExpectations(self->_mockESApi.get());
//   ::testing::Mock::VerifyAndClearExpectations(self->_mockSerializer.get());
//   ::testing::Mock::VerifyAndClearExpectations(self->_mockWriter.get());
